...
!!st5
!!an
  USE usrdef_nam , ONLY : rn_abp, ln_mir_h
!!an
...
!!                             ony on r1_e1e2t so the divergence is impacted only (and not domvvl)
           DO jj=2,jpj
             DO ji = 2,jpi
               IF (                 ( umask(ji,jj,1) + umask(ji-1,jj,1)   &
                  &                 + vmask(ji,jj,1) + vmask(ji,jj-1,1)) == 2._wp  )  THEN
                     r1_e1e2t(ji,jj) = r1_e1e2t(ji,jj) * 2._wp
                     ! e3t_0(ji,jj,1) = -1._wp  ! test
              ENDIF
            END DO
           END DO
           WRITE(numout,*) 'domain : key_halfdiv half T cell r1_e1e2t (happen only in divhor when no tracer) '
      ENDIF
     !
      !
