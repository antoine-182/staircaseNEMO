...
!!an
   LOGICAL, PUBLIC ::   ln_dynadv_cen4  !: flux form - 4th order centered scheme flag
!an
...
!!an
   INTEGER, PUBLIC, PARAMETER ::   np_FLX_c4  = 4   ! flux   form : 2nd order centered scheme
!!an
   !!----------------------------------------------------------------------
   !! NEMO/OCE 4.0 , NEMO Consortium (2018)
   !! $Id: dynadv.F90 12377 2020-02-12 14:39:06Z acc $
...
CONTAINS
   ...
   SUBROUTINE dyn_adv( kt, Kbb, Kmm, puu, pvv, Krhs )
      ...
      CASE( np_FLX_c4  )
         CALL dyn_adv_cen4( kt,                 Kmm, puu, pvv, Krhs )    ! 4th order centered scheme
      END SELECT
      !
   ...
   SUBROUTINE dyn_adv_init
      ...
      !
      NAMELIST/namdyn_adv/ ln_dynadv_OFF, ln_dynadv_vec, nn_dynkeg, ln_dynadv_cen2, ln_dynadv_ubs, ln_dynadv_cen4
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namdyn_adv in configuration namelist' )
      ...
      !
      IF( ln_mir_adv ) nn_dynkeg = nkeg_C2_mirror
      !
...
!!an
         WRITE(numout,*) '                 4th order centred scheme              ln_dynadv_cen4 = ', ln_dynadv_cen4
!!an
      ENDIF
      ...
      IF( ln_dynadv_cen4 ) THEN   ;   ioptio = ioptio + 1   ;   n_dynadv = np_FLX_c4    ;   ENDIF

      IF( ioptio /= 1 )   CALL ctl_stop( 'choose ONE and only ONE advection scheme' )
      IF(   nn_dynkeg /= nkeg_C2     .AND. nn_dynkeg /= nkeg_HW  .AND.   &
        &   nn_dynkeg /= nkeg_C2_mirror .AND. &
        &   nn_dynkeg /= nkeg_RN  )   CALL ctl_stop( 'KEG scheme wrong value of nn_dynkeg' )
      ...
      IF(lwp) THEN                    ! Print the choice
         ...
         CASE( np_VEC_c2  )   ;   WRITE(numout,*) '   ==>>>   vector form : keg + zad + vor is used'
            ...
            IF( nn_dynkeg == nkeg_RN  )   WRITE(numout,*) '              with Renner correction keg scheme'
         CASE( np_FLX_c2  )   ;   WRITE(numout,*) '   ==>>>   flux form   : 2nd order scheme is used'
         CASE( np_FLX_ubs )   ;   WRITE(numout,*) '   ==>>>   flux form   : UBS       scheme is used'
!!an
         CASE( np_FLX_c4  )   ;   WRITE(numout,*) '   ==>>>   flux form   : 4th order scheme is used'
!!an
         END SELECT
      ENDIF
      !
