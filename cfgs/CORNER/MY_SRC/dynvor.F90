...
# if defined key_bvp
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e1e2f penalised'
    ! ENDIF
    ! zr1_e1e2f(:,:) = r1_rpof(:,:,1) * zr1_e1e2f(:,:)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e3t depenalised for geostrophy'
    ! ENDIF
    ! ze3t(:,:,:,Kmm) = r1_rpo (:,:,:) * ze3t(:,:,:,Kmm)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e3u, e3v depenalised for geostrophy'
    ! ENDIF
    ! ze3u(:,:,:,Kmm) = r1_rpou(:,:,:) * ze3u(:,:,:,Kmm)
    ! ze3v(:,:,:,Kmm) = r1_rpov(:,:,:) * ze3v(:,:,:,Kmm)
    !
    !!an un équivalent serait bt doublement pénalisée
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e3t penalised (^2) for geostrophy'
    ! ENDIF
    ! ze3t(:,:,:,Kmm) = rpo (:,:,:) * ze3t(:,:,:,Kmm)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e3t penalised (^3), e3u,e3v (^2) for geostrophy'
    ! ENDIF
    ! ze3t(:,:,:,Kmm) = rpo (:,:,:) * ze3t(:,:,:,Kmm) * rpo (:,:,:)
    ! ze3u(:,:,:,Kmm) = rpou(:,:,:) * ze3u(:,:,:,Kmm)
    ! ze3v(:,:,:,Kmm) = rpov(:,:,:) * ze3v(:,:,:,Kmm)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_enT : e3t,e3u,e3v depenalised'
    ! ENDIF
    ! ze3t(:,:,:,Kmm) = r1_rpo (:,:,:) * ze3t(:,:,:,Kmm)
    ! ze3u(:,:,:,Kmm) = r1_rpou(:,:,:) * ze3u(:,:,:,Kmm)
    ! ze3v(:,:,:,Kmm) = r1_rpov(:,:,:) * ze3v(:,:,:,Kmm)
    !
    IF( kt == nit000 ) THEN
      WRITE(numout,*) 'vor_enT : e3t,e3u,e3v penalised'
    ENDIF
# endif
!
...
# if defined key_bvp
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_ene : e1e2f penalised'
    ! ENDIF
    ! zr1_e1e2f(:,:) = r1_rpof(:,:,1) * zr1_e1e2f(:,:)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_ene : e3f penalised for geostrophy'
    ! ENDIF
    ! ze3f(:,:,:) = rpof(:,:,:) * ze3f(:,:,:)
    !
    ! IF( kt == nit000 ) THEN
    ! WRITE(numout,*) 'vor_ene : e3u, e3v DEpenalised for geostrophy'
    ! ENDIF
    ! ze3u(:,:,:,Kmm) = r1_rpou(:,:,:) * ze3u(:,:,:,Kmm)
    ! ze3v(:,:,:,Kmm) = r1_rpov(:,:,:) * ze3v(:,:,:,Kmm)
# endif
      ...
      DO jk = 1, jpkm1                                 ! Horizontal slab
         ...
         CASE ( np_CRV )                           !* Coriolis + relative vorticity
            ...
            IF( ln_dynvor_msk ) THEN                     ! mask the relative vorticity (NOT the Coriolis term)
               DO_2D_10_10
!!an caution Coriolis only penalised
                 ! zwz(ji,jj) = ( zwz(ji,jj) - ff_f(ji,jj) ) * fmask(ji,jj,jk) + ff_f(ji,jj) * r1_rpof(ji,jj,1)
!!an
...
!!anAD remplacer pu par uu(Nnn) et on fait correctement la direction alternée
   ...
   SUBROUTINE vor_ens( kt, Kmm, kvor, pu, pv, pu_rhs, pv_rhs )
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zr1_e1e2f            ! 2D workspace penalised
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   ze3f, z3d                 ! 3D workspace penalised
      REAL(wp), DIMENSION(jpi,jpj,jpk,3) ::   ze3u, ze3v         ! 4D workspace penalised
    !!----------------------------------------------------------------------
      !
      IF( kt == nit000 ) THEN
...
# if defined key_bvp
      ! IF( kt == nit000 ) THEN
      ! WRITE(numout,*) 'vor_ens : e1e2f penalised'
      ! ENDIF
      ! zr1_e1e2f(:,:) = r1_rpof(:,:,1) * zr1_e1e2f(:,:)
      !
      ! IF( kt == nit000 ) THEN
      ! WRITE(numout,*) 'vor_ens : e3f depenalised for geostrophy'
      ! ENDIF
      ! ze3f(:,:,:) = r1_rpof(:,:,:) * ze3f(:,:,:)
      !
      ! IF( kt == nit000 ) THEN
      ! WRITE(numout,*) 'vor_ens : e3u, e3v DEpenalised for geostrophy'
      ! ENDIF
      ! ze3u(:,:,:,Kmm) = r1_rpou(:,:,:) * ze3u(:,:,:,Kmm)
      ! ze3v(:,:,:,Kmm) = r1_rpov(:,:,:) * ze3v(:,:,:,Kmm)
      !
      IF( kt == nit000 ) THEN
      WRITE(numout,*) 'vor_ens : e3f,e3u,e3v penalised'
      ENDIF
      !
      ! IF( kt == nit000 ) THEN
      ! WRITE(numout,*) 'vor_ens : weird stuff for free-slip'
      ! ENDIF
      ! WHERE(rpof(:,:,:) <= 0.5_wp)
      !   z3d(:,:,:) = 0._wp
      ! END WHERE
...
!!an
      ...
      IF( ln_vorlat .AND. ( ln_dynvor_ene .OR. ln_dynvor_ens .OR. ln_dynvor_mix .OR. ln_dynvor_een) ) THEN
#if defined key_vorlat_ns
IF(lwp) WRITE(numout,*) '      key_vorlat_ns           '
         DO_3D_10_10( 1, jpk )
            IF(    tmask(ji,jj+1,jk) + tmask(ji+1,jj+1,jk)              &
               & + tmask(ji,jj  ,jk) + tmask(ji+1,jj  ,jk) == 3._wp )   fmask(ji,jj,jk) = 2._wp
         END_3D
#else
          DO_3D_10_10( 1, jpk )
             IF(    tmask(ji,jj+1,jk) + tmask(ji+1,jj+1,jk)              &
