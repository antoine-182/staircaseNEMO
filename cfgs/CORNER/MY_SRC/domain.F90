...
#  include "do_loop_substitute.h90"
   ...
   !! $Id: domain.F90 13416 2020-08-20 10:10:55Z gm $
...
!reset namelist for SAS
      ...
      CALL dom_msk( ik_top, ik_bot )    ! Masks
                                        ! Penalisation fields
      !
!!an no difference in freeslip of course
      ! WHERE (tmask(:,:,:)==0)
      !   e3t_0(:,:,:) = rn_abp * e3t_0(:,:,:)
      ! END WHERE
!!an
# if defined key_bvp
      ...
      !  done on the e30 as a trick.
      ! -------------------------------------------------
      ! WHERE( tmask(:,:,:) /= 0._wp )
      !    e3t_0(:,:,:) = rpo(:,:,:) * e3t_0(:,:,:)
      ! END WHERE
      !
      ! La valeur des e3 dans la terre (premier point terre touchant la mer)
      ! n'impacte pas la dynamique même si e3 = e30 + ssh
      ! les e3 à a côte sont multpilié par des vitesses toujours masquées
      !
      WRITE(numout,*) 'domain : e30 penalised !'
      e3t_0(:,:,:) = rpo (:,:,:) * e3t_0(:,:,:)
      e3u_0(:,:,:) = rpou(:,:,:) * e3u_0(:,:,:)
      e3v_0(:,:,:) = rpov(:,:,:) * e3v_0(:,:,:)
#if defined key_sca
      e3f_0(:,:,:) = rpof(:,:,:) * e3f_0(:,:,:)
#endif
      ! ! jpfillcopy mode copy the communication layer (kinf od periodic over the basin)
      CALL lbc_lnk_multi( 'domain', e3t_0,  'T', 1._wp, e3u_0,  'U', 1._wp ,      &
              &                     e3v_0,  'V', 1._wp, e3f_0,  'F', 1._wp,       &
              &           kfillmode=jpfillcopy )
!       !
      ! WRITE(numout,*) 'domain : e30 not penalised !'
      ! WRITE(numout,*) 'e1e2f not penalised'
      !!! e3f n'est pas pénalisé mais les e1e2f !
      ! e1e2f   (:,:) =    rpof(:,:,1) *    e1e2f(:,:)
      ! r1_e1e2f(:,:) = r1_rpof(:,:,1) * r1_e1e2f(:,:)
      ! WRITE(numout,*) 'e1e2f penalised !'
      ! CALL lbc_lnk_multi( 'domain', e1e2f,  'F', 1._wp, r1_e1e2f  , 'F', 1._wp,    &
      !         &                                         kfillmode=jpfillcopy )
      !
      ! r1_e1e2t(:,:) = r1_rpo(:,:,1)*r1_e1e2t(:,:)
      ! CALL lbc_lnk( 'domain', r1_e1e2t,  'T', 1._wp, kfillmode=jpfillcopy )
      !
#endif
!!an45 Ligne de cote a 45deg : r1_e1e2t *= ( mi(umask) + mj(vmask) ) /2
!!                             ony on r1_e1e2t so the divergence is impacted only (and not domvvl)
     ! DO_2D_10_10
     ...
     !    ENDIF
     ! END_2D
     ! WRITE(numout,*) '   an45 half T cell r1_e1e2t '
     ! CALL lbc_lnk_multi( 'domain', e1e2t,  'T', 1._wp, r1_e1e2t,  'T', 1._wp  )
     ! zcoeff = 0.25_wp * (   umask(ji,jj+1,1) + umask(ji+1,jj+1,1)   &
     !    &                 + vmask(ji,jj  ,1) + vmask(ji+1,jj  ,1)   )
     !    IF ( zcoeff /= 0._wp )   THEN
