MODULE dynldf_lap_blp
   ...
   !
!!an
  USE usrdef_nam , ONLY : nn_dynldf_lap_typ, nn_fsp, ln_dynldf_lap_PM    ! use laplacian parameter
   ! USE usrdef_nam , ONLY : nn_dynldf_lap_typ   ! use laplacian parameter
   ! USE dommsk, ONLY : rn_shlat
   !
   IMPLICIT NONE
   PRIVATE
...
!!anSYM
   ...
   INTEGER, PUBLIC, PARAMETER ::   np_dynldf_lap_rot_noh  = 12         ! div-rot   laplacian
   INTEGER, PUBLIC, PARAMETER ::   np_dynldf_lap_sym       = 2         ! symmetric laplacian (Griffies&Hallberg 2000)
   INTEGER, PUBLIC, PARAMETER ::   np_dynldf_lap_sym_noh  = 22         ! symmetric laplacian (Griffies&Hallberg 2000)
   INTEGER, PUBLIC, PARAMETER ::   np_dynldf_lap_con       = 3         ! conventional laplacian (cartesian)
   INTEGER, PUBLIC, PARAMETER ::   np_dynldf_lap_con_noh   = 32         ! conventional laplacian (cartesian)
            ...
            DO jk = 1, jpkm1                                 ! Horizontal slab
               ...
               DO_2D_00_00
                  ...
                  pv_rhs(ji,jj,jk) = pv_rhs(ji,jj,jk) + zsign * r1_e1e2v(ji,jj) / e3v(ji,jj,jk,Kmm)        &
                   ...
                   !
                   !
                   !! way to use it even without obstacles
               ...
               ! devrait converger vers le même flux car e3u/e3t -> 1
                 IF ( nn_fsp==1 .AND. ln_dynldf_lap_PM ) THEN
             ...
             DO jk = 1, jpkm1                                 ! Horizontal slab
                ...
                DO_2D_00_00
                   ...
                   pv_rhs(ji,jj,jk) = pv_rhs(ji,jj,jk) + zsign   &
                    ...
                    !
                    IF ( nn_fsp==1 .AND. ln_dynldf_lap_PM ) THEN ! à mettre hors de la boucle do
                      IF ( (     tmask(ji  ,jj+1,1) + tmask(ji+1,jj+1,1)  &
                           &   + tmask(ji  ,jj  ,1) + tmask(ji+1,jj  ,1)  ) == 3._wp) THEN ! salient angle
                          !
                          IF      ( tmask(ji  ,jj  ,1)==0._wp ) THEN   ! South
...
!!an give the good trends but not really clean
! IF ( nn_dynldf_lap_typ == np_dynldf_lap_sym ) THEN
!   IF ( ln_obstacle .AND. ln_dynldf_lap_PM ) THEN                      !==  Mirror condition  ==!
!     DO jk = 1, jpkm1
!         DO jj = 1, jpjm1
!            DO ji = 1, jpim1
!               IF ( (     tmask(ji  ,jj+1,1) + tmask(ji+1,jj+1,1)  &
!                    &   + tmask(ji  ,jj  ,1) + tmask(ji+1,jj  ,1)  ) == 3._wp) THEN ! salient angle
!                   !
!                   IF      ( tmask(ji  ,jj  ,1)==0._wp ) THEN
!                     zu(ji,jj,jk) =2._wp * pu(ji,jj+1,jk) ; zv(ji,jj,jk) = 2._wp * pv(ji+1,jj,jk)
!                   ELSE IF ( tmask(ji+1,jj  ,1)==0._wp ) THEN
!                     zu(ji,jj  ,jk) = 2._wp * pu(ji,jj+1,jk) ; zv(ji+1,jj,jk) = 2._wp * pv(ji,jj,jk)
!                   ELSE IF ( tmask(ji+1,jj+1,1)==0._wp ) THEN
!                     zu(ji,jj+1,jk) = 2._wp * pu(ji  ,jj,jk) ; zv(ji+1,jj,jk) =2._wp * pv(ji,jj,jk)
!                   ELSE IF ( tmask(ji  ,jj+1,1)==0._wp ) THEN
!                     zu(ji,jj+1,jk) = 2._wp * pu(ji,jj,jk) ; zv(ji,jj,jk) = 2._wp * pv(ji+1,jj,jk)
!                   END IF
!                   !
!               END IF
!         END DO
...
! IF ( ln_obstacle .AND. ln_dynldf_lap_PM ) THEN                      !==  Mirror condition  ==!
  ...
  !   DO jj = 2, jpj
  !      DO ji = 2, jpi
  !     ! In freeslip hé ! (so fmask==0 on the coast (2 in noslip))
  !     IF ( (     fmask(ji-1,jj  ,1) + fmask(ji  ,jj  ,1)  &
  !          &   + fmask(ji-1,jj-1,1) + fmask(ji  ,jj-1,1)  ) == 1._wp) THEN ! inner angle
  !       !
  !       IF      ( fmask(ji  ,jj  ,1)==1._wp ) THEN
  !         zu(ji-1,jj,jk) =-pv(ji  ,jj,jk) ; zv(ji,jj-1,jk) =-pu(ji,jj,jk)
  !         zu(ji  ,jj-1,jk)= pu(ji  ,jj,jk) ; zv(ji-1,jj,jk)= pv(ji,jj,jk) ! double mirror
  !       ELSE IF ( fmask(ji  ,jj-1,1)==1._wp ) THEN
  !         zu(ji-1,jj  ,jk) = pv(ji  ,jj-1,jk) ; zv(ji,jj,jk) = pu(ji,jj,jk)
  !         zu(ji  ,jj+1,jk)= pu(ji,jj,jk) ; zv(ji-1,jj-1,jk)= pv(ji,jj-1,jk) ! double mirror
  !       ELSE IF ( fmask(ji-1,jj-1,1)==1._wp ) THEN
  !         zu(ji,jj  ,jk) =-pv(ji  ,jj-1,jk) ; zv(ji  ,jj,jk) =-pu(ji-1,jj,jk)
  !         zu(ji-1,jj+1,jk)= pu(ji-1,jj,jk) ; zv(ji+1,jj-1,jk)= pv(ji,jj-1,jk) ! double mirror
  !       ELSE IF ( fmask(ji-1,jj  ,1)==1._wp ) THEN
  !         zu(ji,jj,jk) = pv(ji,jj-1,jk) ; zv(ji  ,jj-1,jk) = pu(ji-1,jj  ,jk)
  !         zu(ji-1,jj-1,jk)= pu(ji-1,jj,jk) ; zv(ji+1,jj,jk)= pv(ji,jj,jk) ! double mirror
  !       END IF
  !       !
  !     END IF
  !   END DO
  ! END DO
!       ! only with symetric tensor hé
!       DO jj = 1, jpjm1
!          DO ji = 1, jpim1
!             IF ( (     tmask(ji  ,jj+1,1) + tmask(ji+1,jj+1,1)  &
!                  &   + tmask(ji  ,jj  ,1) + tmask(ji+1,jj  ,1)  ) == 3._wp) THEN ! salient angle
