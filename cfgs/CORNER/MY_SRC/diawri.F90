...
!      CALL histwrite( nid_T, "sobowlin", it, zw2d          , ndim_hT, ndex_hT )   ! ???
   ...
   SUBROUTINE dia_wri_state( Kmm, cdfile_name )
      ...
      !!
      INTEGER :: inum, jk, ji, jj
!!st18  TBR
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: ze3t, zgdept,zwz      ! 3D workspace !!st patch to use substitution
      !!----------------------------------------------------------------------
      !
      IF(lwp) WRITE(numout,*)
...
!!st19 TBR
      ...
      END DO
      DO_2D_10_10
         zwz(ji,jj,1) = (  e2v(ji+1,jj  ) * vv(ji+1,jj  ,1,Kmm) - e2v(ji,jj) * vv(ji,jj,1,Kmm)    &
            &            - e1u(ji  ,jj+1) * uu(ji  ,jj+1,1,Kmm) + e1u(ji,jj) * uu(ji,jj,1,Kmm)  ) &
            &          * r1_e1e2f(ji,jj)
        zwz(ji,jj,1) = zwz(ji,jj,1) * fmask(ji,jj,1)
      END_2D
!!st19
#if defined key_si3
     ...
     CALL iom_open( TRIM(cdfile_name), inum, ldwrt = .TRUE. )
      ...
      ENDIF
      IF( ALLOCATED(fmask) ) THEN
         CALL iom_rstput( 0, 0, inum,  'fmask', fmask              )    ! ahmt at u-point
         CALL iom_rstput( 0, 0, inum,  'relvor', zwz              )    ! ahmt at u-point
      ENDIF
