MODULE ldfdyn
   ...
   !
!!an
   USE usrdef_nam , ONLY : ln_dynldf_lap_PM, nn_fsp, rn_fsp, nn_dynldf_lap_typ, nn_fsp, ln_hdiv_AD, rn_theta
   USE dommsk , ONLY : ln_vorlat_ldf, rn_vorlat_ldf
!!an
   !
   IMPLICIT NONE
   PRIVATE
         ...
         IF( .NOT.l_ldfdyn_time ) THEN             !* No time variation
            IF(     ln_dynldf_lap ) THEN                 !   laplacian operator (mask only)
               ! classic boundary condition on the viscosity coefficient
                ...
                WRITE(numout,*) ' classic ahmt tmasked '
                ahmf(:,:,1:jpkm1) =       ahmf(:,:,1:jpkm1)   * fmask(:,:,1:jpkm1)
                WRITE(numout,*) ' classic ahmf fmasked '
!!an            !
                WRITE(numout,*) '   ln_dynldf_lap_PM = ',ln_dynldf_lap_PM
                WRITE(numout,*) '   nn_dynldf_lap_typ = ',nn_dynldf_lap_typ
                !
!!an
                IF( ln_vorlat_ldf ) THEN
                   WRITE(numout,*) 'ldfdyn : In angles, shlat vorticity (ln_vorlat_ldf = T)'
                   DO jj = 1, jpjm1
                      DO ji = 1, jpim1
                        ! ahmf est défini avec le fmask avant ln_vorlat
                        IF(    tmask(ji,jj+1,1) + tmask(ji+1,jj+1,1)              &
                           & + tmask(ji,jj  ,1) + tmask(ji+1,jj  ,1) == 3._wp ) THEN
                           ahmf(ji,jj,1) = rn_vorlat_ldf * ( 0.5_wp*rn_Uv*rn_Lv )
                        ENDIF
                     END DO
                   END DO
                   !
