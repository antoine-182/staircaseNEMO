...
# ==========
#
# $Id: all_functions.sh 11161 2019-06-20 16:55:23Z mathiot $
#
#   * creation
#-
...
set_valid_dir () {
    LANG=en_US
    REVISION_NB=`LC_MESSAGES=${LANG} ${SVN_CMD} info ${SETTE_DIR}/.. | grep "Last Changed Rev" | awk '{print $NF}'`
    if [ ${#REVISION_NB} -eq 0 ]
    then
    ...
    [ `${SVN_CMD} status -q ${SETTE_DIR}/../{cfg,tests,src} | wc -l` -ge 1 ] && REVISION_NB=${REVISION_NB}+
