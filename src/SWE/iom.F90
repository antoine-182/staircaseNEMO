...
#  include "do_loop_substitute.h90"
   ...
   !! $Id: iom.F90 12489 2020-02-28 15:55:11Z davestorkey $
...
CONTAINS
   ...
   SUBROUTINE iom_init( cdname, fname, ld_tmppatch, ld_closedef ) 
      ...
      IF( TRIM(cdname) == TRIM(cxios_context)//"_crs" ) THEN  
         ...
         CALL set_grid( "F", glamf_crs, gphif_crs, .FALSE., .FALSE. ) 
         ...
         IF( ln_cfmeta .AND. .NOT. llrst_context) THEN   ! Add additional grid metadata
            ...
            CALL iom_set_domain_attr("grid_F", area = e1f_crs(nldi:nlei, nldj:nlej) * e2f_crs(nldi:nlei, nldj:nlej))
            ...
            CALL set_grid_bounds( "F", glamt_crs, gphit_crs, glamf_crs, gphif_crs )
...
#if defined key_iomput
!!an juste pour compiler xios2.0
