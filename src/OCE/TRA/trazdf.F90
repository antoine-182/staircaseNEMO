   ...
   SUBROUTINE tra_zdf_imp( kt, kit000, cdtype, p2dt, Kbb, Kmm, Krhs, pt, Kaa, kjpt )
      ...
      DO jn = 1, kjpt                                 !  tracer loop  !
         ...
         IF(  ( cdtype == 'TRA' .AND. ( jn == jp_tem .OR. ( jn == jp_sal .AND. ln_zdfddm ) ) ) .OR.   &
            ...
            IF( cdtype == 'TRA' .AND. jn == jp_tem ) THEN   ;   zwt(:,:,2:jpk) = avt(:,:,2:jpk)  !!an diffusion de sel ou temp
            ELSE                                            ;   zwt(:,:,2:jpk) = avs(:,:,2:jpk)
            ENDIF
            zwt(:,:,1) = 0._wp  !!an diffusion nul en surface
            !
            ...
            ENDIF
            !
!!an zwt coeff de diffusion verticale
!! key zwt = zwt*rpow
