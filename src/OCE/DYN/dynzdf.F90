...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: dynzdf.F90 12489 2020-02-28 15:55:11Z davestorkey $
...
!!an e3 after ou now
      ...
      !
!!an
! oui car avm est sorti dans zdfi
!! key avm=avm*rpo
!!an
         ...
         CASE( np_lap_i )           ! rotated lateral mixing: add its vertical mixing (akzu)
            DO_3D_00_00( 1, jpkm1 )
               ze3ua =  ( 1._wp - r_vvl ) * e3u(ji,jj,jk,Kmm)    &
                  ...
                  !!an 
                  !! kz caculé au point w pour speed et tracers (fermeture)
                  !! i inferieure, s superieure
                  !! avm definit au point w donc ramené au point u
                  !! soit *rpou SOIT (avm*rpow + avm*rpow) [le 1/2 n'est pas nécessaire],  (2) wins
                  !! attention le kz est fortement non lineaire, donc on a envie de ponderer la diffusion par le volume fluide
                  !! FLUX diffusif : on penalise les surfaces de contact
                  !!an
